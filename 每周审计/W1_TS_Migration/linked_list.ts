...
class ListNode {
    public data: number;
    public next: ListNode | null; // 在 TS 中，空指针用 null 表示，| 符号是联合类型
...
// 2. 定义单链表管理类
class LinkedList {
    private head: ListNode | null; // 头指针
    private size: number;          // 链表长度
 ...
 * @param element 要插入的值
 */
public insertAt(index: number, element: number): boolean {
