...
- 像 `python_basics_day0.py` 到 `hello_cli.py` 这类入门练习，Codex 可以先给练习文件，我负责修改、运行、解释和记录。
- 从真实小工具开始，核心逻辑尽量由我先手敲。当前最典型的例子是 `scripts/site_probe.py` 里的 `probe_url(url)`。
- 以后像证书检测核心函数、数据库核心逻辑、认证逻辑、项目里的关键业务判断，都优先由我先写第一版。
- Codex 可以做的事：拆步骤、解释概念、提示下一小步、检查报错、做 review、整理格式、补文档、补测试建议。
- Codex 默认不要直接替我写完整核心答案，除非我明确说“先演示给我看”或者“这次你直接写一版给我学习”。
- 如果我自己没写出来，Codex 可以先给最小提示版，不直接给完整最终实现。
- 判断是否算掌握，不看是不是一个字一个字手敲，而看 4 点：我能不能自己改、我能不能解释、我删掉后能不能重写核心部分、我遇到小 bug 能不能自己修回来。
