...
Stage: Week 2 - Structured data, Day 1.

Today's target:

- [x] Run `scripts/python_week2_day1.py`.
- [x] Change `name`.
- [x] Change `url`.
- [x] Change `check_ssl`.
- [x] Change `interval_minutes`.
- [x] Add one new field such as `owner`.

Notes:

- What I tried: 我运行了 `python .\scripts\python_week2_day1.py`，把 `name` 改成了 `lenxuan`，把 `url` 改成了 `https://lenxuan.com`，把 `check_ssl` 改成了 `False`，把 `interval_minutes` 改成了 `2`，并且加上了 `owner` 字段。
- What failed: 暂时没有报错。
- What I understood after fixing it: 字典可以把一组相关的数据放在一起，比如站点名字、网址、是否检查 HTTPS、检查间隔和负责人。现在我会把字典理解成“一个带名字的数据包”。
