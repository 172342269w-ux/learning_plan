...
Stage: Week 1 - first real script, `site_probe.py`.

Today's target:

- [x] Implement `probe_url(url)`.
- [x] Read a URL from `sys.argv[1]`.
- [x] Return an HTTP status code.
- [x] Print `OK`, `WARN`, or `FAIL`.
- [x] Handle usage errors and request failures.

Notes:

- What I tried: 我实现了 `probe_url(url)`，使用 `urllib.request.urlopen(url, timeout=5)` 请求 URL，并返回 HTTP 状态码。
- What failed: 本机 Python 访问 `https://example.com` 时出现证书链验证失败，所以这一步先用 `http://example.com` 验证 HTTP 检测逻辑；访问不存在的域名时也可能超时。
- What I understood after fixing it: `site_probe.py` 把 Week 1 的知识串起来了：`sys.argv[1]` 负责接收 URL，`probe_url` 负责请求并 `return` 状态码，`if 200 <= status_code < 400` 负责判断是否 `OK`。4xx/5xx 这类 HTTP 错误也可以当成状态码处理，不应该直接让程序崩掉。
